...
export type RetailerPortal = 'Home Depot' | "Lowe's" | "HD / Lowe's";

/** Which big-box portal this job likely belongs in. */
export function retailerPortalLabel(source: string): RetailerPortal {
  const text = String(source || '');
  if (/\b(?:the\s+)?home\s+depot\b/i.test(text)) return 'Home Depot';
  if (/\blowe['’]?s\b/i.test(text)) return "Lowe's";
  return "HD / Lowe's";
}
...
export function heaterTypeNeedsAi(source: string): boolean {
  const label = heaterTypeLabel(source);
  if (!label) return /\S/.test(source);
  if (/\bModel\s+[A-Z0-9-]/i.test(label) && RETAILER.test(label)) return false;
  return true;
}
...
export function wrapInstallDescription(
  description: string,
  rows: number,
  maxLen = 78
): string[] {
